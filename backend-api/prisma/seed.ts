...
async function main() {
  console.log('--- RESETTING AND SEEDING DATABASE (v4.21) ---');
  ...
  // 3. Default Permissions (Ensuring modules are visible)
  const defaultPermissions = [
    ...
    { role: 'ROLE_CONTABILIDAD', key: 'menu:payments_view', enabled: false },
  ...
  for (const perm of defaultPermissions) {
    await prisma.permission.upsert({
      where: { 
        id: await prisma.permission.findFirst({
          where: { roleName: perm.role, permissionKey: perm.key }
        }).then(p => p?.id || -1)
      },
  ...
  // 4. Dummy Messages (from CasaVida)
  console.log('--- SEEDING DUMMY MESSAGES ---');
  
  // Find the actual vendedor user to get their real ID
  ...
  const recepcionUser = await prisma.user.findUnique({
    where: { username: 'recepcion' }
  });

  if (!vendedorUser || !adminUser || !recepcionUser) {
    throw new Error('Test users were not created successfully!');
  }

  // Create a sample ticket for the messages
  ...
  const dummyMessages = [
    ...
    { userId: vendedorUser.id, contenido: 'Por favor confirmen si el lote L-01 sigue disponible, tengo un cliente interesado.' },
  ];
  ...
  console.log('Seed completed successfully! You can now log in with:');
  console.log(' - Admin: admin / password123');
  console.log(' - Recepcion: recepcion@test.com / password123');
  console.log(' - Vendedor: vendedor@test.com / password123');
}
